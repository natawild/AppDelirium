...
plot_confusion_matrix(
    ...
    title="Matriz de confusão sem seleção de variáveis (RF_SFM)",
)
...
# summarize scores
print('RF antes seleção variáveis: f1=%.3f auc=%.3f' % (lr_f1, lr_auc))
# plot the precision-recall curves
no_skill = len(y_test[y_test==1]) / len(y_test)
pyplot.plot(lr_recall, lr_precision, label='AUC ' + str(lr_auc))
# axis labels
pyplot.title('Curva Especificidade-Sensibilidade antes seleção de variáveis (RF_SFM_THRE=0,002)')
...
###############################################################################
#             Select most important features with RF                      #
###############################################################################
...
sel = SelectFromModel(rf, threshold=0.02)
...
plt.title('Variáveis selecionadas e respetiva importância (RF_SFM_THRE=0,02)')
plt.show()
...
pprint(feat_selector)

# find all relevant features - 5 features should be selected
relefeatr= feat_selector.fit(X_train, y_train)
...
X_filtered = relefeatr.transform(X_train)


final_features = list()
indexes = np.where(relefeatr.support_ == True)
...
plt.xlabel('1 - Especificidade (FFP)')
plt.ylabel('Sensibilidade (FVP)')
plt.title('Curva ROC das variáveis selecionadas (RF_SFM_THRE=0,002)')
# show the legend
plt.legend()
plt.show()
...
plot_confusion_matrix(
    confusion_matrix(y_test, y_pred),
    classes=["0 - Sem Delirium", "1 - Delirium"],
    title="Matriz de confusão após seleção de variáveis (RF_SFM_THRE=0,002)",
)
...
pyplot.plot(lr_recall, lr_precision, label='AUC ' + str(lr_auc))
# axis labels
pyplot.title('Curva Especificidade-Sensibilidade após seleção de variáveis (RF_SFM_THRE=0,002)')
pyplot.xlabel('Sensibilidade')
pyplot.ylabel('Especificidade')
# show the legend
pyplot.legend()
...
plt.xlabel('1 - Especificidade (FFP)')
plt.ylabel('Sensibilidade (FVP)')
plt.title('Curva ROC após tuning (RF_SFM)')
# show the legend
plt.legend()
plt.show()
...
plot_confusion_matrix(
    ...
    classes=["0 - Sem Delirium", "1 - Delirium"],
    title="Matriz de confusão após tuning (RF_SFM)",
)
...
pyplot.plot(lr_recall, lr_precision, label='AUC ' + str(lr_auc))
# axis labels
pyplot.title('Curva Especificidade-Sensibilidade após afinação (RF_SFM- THRE=0,002)')
pyplot.xlabel('Sensibilidade')
pyplot.ylabel('Especificidade')
# show the legend
pyplot.legend()
