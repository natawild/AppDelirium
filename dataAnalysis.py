...
"""
fig=plt.figure()
plt.plot(dadosDelirium['Idade'], dadosDelirium['ResultDelirium'] , color = 'blue', lw = 2) 

plt.xlabel("Delirium")
plt.ylabel("Idade")
plt.title("Delirium vs Idade")
fig.savefig('test.png')

sns.distplot(dadosDelirium['Idade'])
sns.barplot(x='Genero', y='Idade', data=dadosDelirium, hue='Genero')
plt.show () 
"""
...
X = dadosDelirium.iloc[:, 1:-1].values
y = dadosDelirium.iloc[:, -1].values

seed = 50  # so that the result is reproducible
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.333, random_state=seed
)
...
plot_confusion_matrix(
    ...
    title="ResultDelirium Confusion Matrix",
)
plt.show()
