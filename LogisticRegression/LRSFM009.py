...
pd.DataFrame.from_dict(sel.get_support()).T.to_csv("RL_SFM009_dict.csv")
...
#C = [1.0, 1.001, 1.01]
C = [int(x) for x in np.arange(start=0.01, stop=1.1, step=0.01)]
#max_iter = [75, 80, 90, 100, 150, 500]
max_iter = [int(x) for x in np.arange(start=2, stop=500, step=1)]
...
#-------------------------------------------GridSearchCV--------------------------------------------------#
'''
random_grid = GridSearchCV(estimator=clf,
