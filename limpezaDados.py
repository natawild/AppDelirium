...
"""
data = pd.read_csv("./dados_apos_p_processamento.csv", na_values="NA")
...
data.plot(kind="density", subplots=True, layout=(39, 39), sharex=False)
pyplot.show()
...
thresholds = arange(0.0, 0.70, 0.01)
...
for t in thresholds:
    ...
    print(X_train.shape)
    X_sel = transform.fit_transform(X_train)
...
print(X_sel.shape, y_train.shape)


# identify outliers in the training dataset
lof = LocalOutlierFactor()
yhat = lof.fit_predict(X_sel)

print("Outliers: ", yhat)

# select all rows that are not outliers
mask = yhat != -1
X_sel, y_train = X_sel[mask, :], y_train[mask]
